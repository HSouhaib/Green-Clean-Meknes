...
import { Search, Shield, User, UserCheck, UserX, Trash2, X, CalendarDays, Award } from 'lucide-react';
...
export function UsersTab() {
  ...
  const { data: userList } = trpc.user.list.useQuery({ page, limit: 20, search: search || undefined, role: roleFilter || undefined });
        ...
        columns={[
          {
            ...
            render: (u) => (
              <div className="flex items-center gap-3">
                ...
                >
                  {u.avatar ? (
                    <img src={u.avatar} alt="" className="w-full h-full rounded-full object-cover" loading="lazy" />
                  ) : (
                    <User size={14} />
                  )}
                </div>
          ...
          },
          {
            ...
            render: (u) => (
              <span
                ...
                style={{ color: u.isActive ? 'var(--accent-green)' : 'var(--accent-terracotta)' }}
              >
          ...
          >
            ...
            <div className="p-4 space-y-6">
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  {selectedUser.avatar ? (
                    <img src={selectedUser.avatar} alt="" className="w-full h-full rounded-full object-cover" loading="lazy" />
                  ) : (
                    selectedUser.name?.charAt(0).toUpperCase() || <User size={20} />
                  )}
                </div>
                <div>
                  ...
                  <p className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>
                    ...
                    {selectedUser.unionId?.startsWith('local:') && (
                      <span className="ml-2 px-1.5 py-0.5 rounded text-[10px]" style={{ background: 'var(--accent-green-muted)', color: 'var(--accent-green)' }}>
                        Local
                      </span>
                    )}
                  </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg" style={{ background: 'var(--bg-surface)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <CalendarDays size={14} style={{ color: 'var(--accent-green)' }} />
                    <span className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Registrations</span>
                  </div>
                ...
                <div className="p-3 rounded-lg" style={{ background: 'var(--bg-surface)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <Award size={14} style={{ color: 'var(--accent-terracotta)' }} />
                    <span className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Attended</span>
                  </div>
                  <p className="text-lg font-medium" style={{ color: 'var(--text-primary)' }}>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={(e) => updateRoleMutation.mutate({ id: selectedUser.id, role: e.target.value })}
              ...
              <div className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'var(--bg-surface)' }}>
                ...
                <button
                  onClick={() => toggleStatusMutation.mutate({ id: selectedUser.id, isActive: !selectedUser.isActive })}
                  disabled={toggleStatusMutation.isPending}
                  className="relative w-12 h-6 rounded-full transition-colors"
                  style={{
                    background: selectedUser.isActive ? 'var(--accent-green)' : 'var(--bg-surface-light)',
                  }}
                >
                  <span
                    ...
                    style={{
                      ...
                      transform: selectedUser.isActive ? 'translateX(1.5rem)' : 'translateX(0)',
                    }}
                  />
                </button>
              </div>
              ...
              {/* Delete User */}
              <div className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.1)' }}>
                <span className="text-sm" style={{ color: 'var(--text-primary)' }}>
                  Delete Account
                </span>
                <button
                    ...
                    {selectedUser.registrations.map((reg) => (
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{
                            background: reg.status === 'attended' ? 'var(--accent-green)' : 'var(--bg-surface-light)',
                            color: reg.status === 'attended' ? 'white' : 'var(--text-secondary)',
                          }}
                        >
                          {reg.status}
                        </span>
