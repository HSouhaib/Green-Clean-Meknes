...
export default function ShareButton({ platform, campaign, size = 18 }: ShareButtonProps) {
  const { t } = useLanguage();
  const [showToast, setShowToast] = useState(false);
  const url = `https://green.meknes/campaigns/${campaign.slug}`;

  const handleClick = () => {
    const text = t('share.text')
      .replace('{title}', campaign.title)
      .replace('{date}', campaign.date)
  ...
  return (
    <>
      ...
      {showToast && (
        <div
          className="fixed bottom-8 left-1/2 -translate-x-1/2 px-6 py-3 rounded-full font-mono text-xs z-[100]"
          style={{
