...
import { X, MapPin, Calendar, Users, Trash2, Sprout } from 'lucide-react';
...
export default function CampaignDetailModal({ campaign, onClose }: CampaignDetailModalProps) {
  ...
  const handleRegister = (e: React.MouseEvent) => {
    ...
    // Always show guest registration form
      ...
      >
        ...
        {campaign.eventDate && !isClosed && (
          <div
            ...
            style={{ background: 'var(--bg-surface)', borderBottom: '1px solid var(--bg-surface-light)' }}
          >
            <CampaignCountdown
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between gap-3">
            <h2
              ...
              style={{ color: 'var(--text-primary)', fontSize: '22px', letterSpacing: '-0.01em' }}
            >
              {title}
            </h2>
            <span
          ...
          {/* Location */}
          <p className="text-sm font-light mt-2 flex items-center gap-1.5" style={{ color: 'var(--text-secondary)' }}>
            <MapPin size={14} />
            {location}
          </p>

          {/* Description */}
          <p className="text-sm font-light mt-4 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
            {description}
          </p>
          ...
          <div className="flex items-center gap-4 mt-5">
            <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
              <Users size={14} />
              <span>{count} {t('campaigns.registered_count')}</span>
            </div>
            {campaign.mapX && campaign.mapY && (
              <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                <MapPin size={14} />
                <span>{campaign.mapX.toFixed(4)}, {campaign.mapY.toFixed(4)}</span>
              </div>
            )}
          </div>

          {/* Impact stats row */}
          {((campaign.statsWasteKg ?? 0) > 0 || (campaign.statsTrees ?? 0) > 0 || (campaign.statsVolunteers ?? 0) > 0 || (campaign.statsNeighborhoods ?? 0) > 0) && (
            <div className="flex flex-wrap items-center gap-4 mt-3">
              {(campaign.statsWasteKg ?? 0) > 0 && (
                <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                  <Trash2 size={14} />
                  <span>{campaign.statsWasteKg ?? 0} {t('campaigns.stats.waste')}</span>
                </div>
              )}
              {(campaign.statsTrees ?? 0) > 0 && (
                <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                  <Sprout size={14} />
                  <span>{campaign.statsTrees ?? 0} {t('campaigns.stats.trees')}</span>
                </div>
              )}
              {(campaign.statsVolunteers ?? 0) > 0 && (
                <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                  <Users size={14} />
                  <span>{campaign.statsVolunteers ?? 0} {t('campaigns.stats.volunteers')}</span>
                </div>
              )}
              {(campaign.statsNeighborhoods ?? 0) > 0 && (
                <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                  <MapPin size={14} />
                  <span>{campaign.statsNeighborhoods ?? 0} {t('campaigns.stats.neighborhoods')}</span>
                </div>
              )}
            </div>
          )}
          ...
          {/* Divider */}
          <div className="mt-5" style={{ borderTop: '1px solid var(--bg-surface-light)' }} />

          {/* Actions row */}
          <div className={`flex items-center mt-5 ${isRtl ? 'flex-row-reverse' : ''}`}>
            {/* Register button */}
            <button
